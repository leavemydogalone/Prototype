...
{
	if (!Super::CanGameplayEffectApply(ActiveGEContainer, GESpec) || !UTurnBasedAbilitySystemBPLibrary::GetTurnSystem(ActiveGEContainer.Owner))
	{
		return false;
	}

	FCustomContextData_TurnBaseEffect Data = UTurnBasedAbilitySystemBPLibrary::GetTurnBasedEffectInstanceDataFromSpec(GESpec);
	return Data.IsValid();
}
...
bool UTurnBasedGameplayEffectComponent::OnActiveGameplayEffectAdded(FActiveGameplayEffectsContainer& ActiveGEContainer, FActiveGameplayEffect& ActiveGE) const
{
	...
	ITurnSystemInterface* const TurnSystem = UTurnBasedAbilitySystemBPLibrary::GetTurnSystem(OwnerASC);
	...
	if (bApplyUninhibited && OwnerASC->IsOwnerActorAuthoritative())
	{
		for (const TSubclassOf<UGameplayEffect>& Effect : EffectToApplyOnUninhibition)
		{
			if (IsValid(Effect))
			{
				UTurnBasedAbilitySystemBPLibrary::ApplyTurnBasedGameEffect(OwnerASC, ActiveGE, Effect, bCopyContextFromSource);
			}
		}
...
void UTurnBasedGameplayEffectComponent::OnTurnChange(const int32 NewTurn, FActiveGameplayEffectHandle Handle) const
{
	...
	FCustomContextData_TurnBaseEffect Data = UTurnBasedAbilitySystemBPLibrary::GetTurnBasedEffectInstanceDataFromSpec(ActiveEffect->Spec);
	if (!Data.IsValid())
	{
	...
	if (bWasInhibited)
	{
		...
		if (OwnerASC->IsOwnerActorAuthoritative())
		{
			for (const TSubclassOf<UGameplayEffect>& Effect : EffectToApplyOnUninhibition)
			{
				...
				UTurnBasedAbilitySystemBPLibrary::ApplyTurnBasedGameEffect(OwnerASC, *ActiveEffect, Effect, bCopyContextFromSource);
			}
		}
	}
...
void UTurnBasedGameplayEffectComponent::OnGameplayEffectRemoved(const FGameplayEffectRemovalInfo& RemovalInfo, UAbilitySystemComponent* ASC, FDelegateHandle DelegateHandle) const
{
	...
	for (const TSubclassOf<UGameplayEffect>& Effect : EffectToApplyOnRemoval)
	{
		...
		UTurnBasedAbilitySystemBPLibrary::ApplyTurnBasedGameEffect(ASC, *RemovalInfo.ActiveEffect, Effect, bCopyContextFromSource);
	}
}
